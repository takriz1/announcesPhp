...
$query = "DELETE FROM `images_selling` where id_img=$id_delete ";
$exec = mysqli_query($conn,$query);
header('location:modifier-produit.php?id='.$id);
}

// 3- Préaparation de la requete
$query = "SELECT * FROM `selling` where id_s=$id ";
...
$px = addslashes($_POST['px']);
	...
	// File upload path
$query_up = "UPDATE `selling` SET `libelle_s`='$lib', `prix_s`='$px', `description_s`='$desc',`id_cat`='$idcat' WHERE id_s=$id ";
	
	
$countfiles = count($_FILES['image']['name']);
for($i=0;$i<$countfiles;$i++){
	$targetDir = "uploads/selling/";
...
$query_img = "INSERT INTO `images_selling`( `libelle_img`, `id_s`) VALUES ( '$name','$id')";
...
<input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="cus_name" name="lib" type="text" required="" placeholder="Libellé" aria-label="Name" value="<?php echo $libelle_s; ?>">
</div>
        <div class="">
<label class="block text-sm text-gray-00" for="cus_name">Prix</label>
<input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="cus_name" name="px" type="text" required="" placeholder="Libellé" aria-label="Name" value="<?php echo $prix_s; ?>">
</div>
...
?>
            <div class="icondelete" style="float:left;width:20%;">
							<img src="uploads/produits/<?php echo $libelle_img; ?>" alt="<?php echo $libelle_s; ?>"   style="width:150px;"/>
							<div class="">
